function createSuccessTestData() {
  ...
  const test1 = {
    testDescription: 'returning 3 when adding 2 + 1', left: 2, right: 1, result: 3,
  };
  const test2 = {
    testDescription: 'returning 3 when adding 1 + 2', left: 1, right: 2, result: 3,
  };
  const test3 = {
    testDescription: 'returning 4 when adding 2 + 2', left: 2, right: 2, result: 4,
  };
  const test4 = {
    testDescription: 'returning 1 when adding 2 + -1', left: 2, right: -1, result: 1,
  };
  const test5 = {
    testDescription: 'returning -1 when adding -2 + 1', left: -2, right: 1, result: -1,
  };
